...
export async function addBlankPage() {
  ...
  if (pageNumberInput.trim() === '') {
    showAlert('Entrada inválida', 'Informe um número de página.');
    return;
  }

  if (pageCountInput.trim() === '') {
    showAlert('Entrada inválida', 'Informe a quantidade de páginas a inserir.');
    return;
  }
  ...
  if (isNaN(position) || position < 0 || position > totalPages) {
    showAlert(
      'Entrada inválida',
      `Informe um número entre 0 e ${totalPages}.`
    );
    return;
  }
  ...
  if (isNaN(pageCount) || pageCount < 1) {
    showAlert(
      'Entrada inválida',
      'Informe uma quantidade válida de páginas (1 ou mais).'
    );
    return;
  }

  showLoader(
    `Adicionando ${pageCount} página${pageCount > 1 ? 's' : ''} em branco...`
  );
  try {
    ...
    showAlert(
      'Erro',
      `Não foi possível adicionar página${pageCount > 1 ? 's' : ''} em branco.`
    );
  } finally {
    hideLoader();
  }
