...
export async function changePermissions() {
  ...
  try {
    ...
    try {
      ...
      throw new Error('Falha no processamento: ' + errorMsg || 'Erro desconhecido');
    }
    ...
    if (!shouldEncrypt) {
      successMessage =
        'PDF descriptografado! Toda a criptografia e restrições foram removidas.';
    }
    ...
    if (error.message === 'INVALID_PASSWORD') {
      showAlert(
        'Senha incorreta',
        'A senha atual informada está errada. Tente novamente.'
      );
    } else if (error.message === 'PASSWORD_REQUIRED') {
      showAlert(
        'Senha obrigatória',
        'Este PDF está protegido por senha. Informe a senha atual para continuar.'
      );
    } else {
      showAlert(
        'Falha no processamento',
        `Ocorreu um erro: ${error.message || 'O PDF pode estar corrompido ou protegido.'}`
      );
    }
  } finally {
    try {
