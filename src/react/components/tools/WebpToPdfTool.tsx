...
export const WebpToPdfTool = () => {
  ...
  useEffect(() => {
    setupFileInputHandler('webp-to-pdf');
  }, []);

  return (
    <div>
      <h2 className="text-2xl font-bold text-white mb-4">
        {t('templates.webpToPdf.title')}
      </h2>
      <p className="mb-6 text-gray-400">
        {t('templates.webpToPdf.description')}
      </p>

      <FileInput multiple accept="image/webp" showControls />

      <div id="file-display-area" className="mt-4 space-y-2"></div>

      <button id="process-btn" className="btn-gradient w-full mt-6">
        {t('templates.webpToPdf.submit')}
      </button>
    </div>
  );
};
