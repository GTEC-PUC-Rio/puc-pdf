...
export const LinearizeTool = () => {
  const { t } = useTranslation('tools');

  useEffect(() => {
    setupFileInputHandler('linearize');
  }, []);

  return (
    <div>
      <h2 className="text-2xl font-bold text-white mb-4">
        {t('templates.linearize.title')}
      </h2>
      <p className="mb-6 text-gray-400">
        {t('templates.linearize.description')}
      </p>
      ...
      <button
        id="process-btn"
        className="hidden btn-gradient w-full mt-6"
        disabled
      >
        {t('templates.linearize.submit')}
      </button>
    </div>
  );
};
