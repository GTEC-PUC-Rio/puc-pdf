...
export const ExtractAttachmentsTool = () => {
  ...
  useEffect(() => {
    setupFileInputHandler('extract-attachments');
  }, []);

  return (
    <div>
      <h2 className="text-2xl font-bold text-white mb-4">
        {t('templates.extractAttachments.title')}
      </h2>
      <p className="mb-6 text-gray-400">
        {t('templates.extractAttachments.description')}
      </p>
      ...
      <button id="process-btn" className="btn-gradient w-full mt-6">
        {t('templates.extractAttachments.submit')}
      </button>
    </div>
  );
};
