...
export const HeicToPdfTool = () => {
  ...
  useEffect(() => {
    setupFileInputHandler('heic-to-pdf');
  }, []);

  return (
    <div>
      <h2 className="text-2xl font-bold text-white mb-4">
        {t('templates.heicToPdf.title')}
      </h2>
      <p className="mb-6 text-gray-400">
        {t('templates.heicToPdf.description')}
      </p>

      <FileInput multiple accept=".heic,.heif" showControls />

      <div id="file-display-area" className="mt-4 space-y-2"></div>

      <button id="process-btn" className="btn-gradient w-full mt-6">
        {t('templates.heicToPdf.submit')}
      </button>
    </div>
  );
};
